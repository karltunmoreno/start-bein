...
function Map(tokenId, inputId, outputId) {
  token.id = tokenId;
...
function search(keyword, output) {
  ...
  let url =
    ...
    token() +
    "AIzaSyBJcM7ofEHCbdcELuNIIZul-lb7rPt1XaU" +
    keyword;
  fetch(url)
    .then((x) => x.json())
    .then((result) => {
      ...
      output.append(
        $("<div/>").html("Click on any of the station to see the detailed AQI")
      );
...
function showStation(station, output) {
  ...
  fetch(url)
    ...
    .then((result) => {
      ...
      output.append(
        $("<div>").html(
          "Station: " + result.data.city.name + " on " + result.data.time.s
        )
      );
